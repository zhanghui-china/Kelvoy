...
同事新增的 `comfyui-bridge/` 提供 Qwen-Image 2.1 与 MiniMax-H3 的 UI/API 工作流，以及 Flask HTTP 包装。Kelvoy 已有 Worker → `services/inference` 的 JSON 协议与 5 分钟调用超时。
...
- 显式传 9:16 与 3–5 秒，不沿用桥接服务的默认参数。模型、工作流哈希/版本、seed、输入参考图哈希和尝试次数写回镜记录；推理耗时由接口返回，当前领域 schema 尚无逐次耗时字段，待成本计量设计与 PRD §6 一起补齐。SQLite 仍是期与任务状态唯一真源。
- `packages/engine` 只计算选图、参数、状态和记录；Worker 注入实际 HTTP 调用与文件归档。每个任务 ID 形成独立产物 key，重生成保留旧文件。
