...
账号（User）只有登录必需的字段 + 一列 `settings`（出片默认值：默认语气 / 默认每镜候选数，设置页写入、新建一期页读来预填，见 §16、M2-15；两项都可缺省；历史 `default_mode` 字段可读，新设置只接受 `per_shot`，新建期忽略旧 `grid` 默认；未设置候选数的新期默认 3，旧期 JSON 缺 candidate_count 时仍按 2 解析）。`settings.onboarding_dismissed_version` 可缺省，取值限 0/1：1 表示账号已手动隐藏当前版本创作指引，0 表示从帮助页重新打开；首页五步完成情况只由账号名下期及镜头状态推导，不另存进度。——积分余额与消耗明细（FR-11）现在加只是空占位，不如不加，要不要加见附录 A。期上的 `estimated_credits` / `credits_used` 两个字段保留不动：填的是成本估算与实际消耗的等价值（口径见 FR-09），不涉及扣费。
...
## 17. 2026-09-26 工作台视觉与入口更新

本轮产品界面以浅蓝灰背景 `#F3F7FC`、白色主面板、`#F7FAFF` 次面板与浅色侧栏、蓝色强调 `#2563FF` 为统一视觉基础。标题与正文使用无衬线字体。桌面侧栏宽 208px；平板以 72px 图标导航展示并保留可访问名称；手机使用 56px 顶栏与菜单抽屉。官网和登录页独立布局，官网首屏使用项目提供的旅行图片。

首页保留真实的继续创作、最近作品和本周记录，并提供角色、目的地、模板、审核四个能力入口。目的地卡片的类型筛选取自已入库数据，图片来自相应地标参考图；没有图片时使用缺图状态。官网图片走公开的目的地素材接口，仅提供该目的地目录实际引用的图片，不开放角色参考图。新建一期将原有完整参数分为主要表单、高级设置与角色选择面板，角色按本人和官方分栏；去创建角色再返回时，浏览器会话内保留尚未提交的表单，成功建期后清除。草稿绑定当前登录账号 ID，切换账号或退出登录会清除，不向其他账号展示；同账号会话到期后重新登录可恢复。从首页目的地卡片进入时，链接指定的目的地优先于旧草稿里的目的地，其余独立创作字段保留；手工填写的自定义季节不会因切换目的地而被覆盖。`/help` 提供与当前功能一致的使用说明。

这些变更只影响展示和新建入口，不改变 `Episode`、`Persona`、`Destination` 的协议或生成/审核状态机。费用和 GPU 估算仍按现有 M0 前口径标注，页面不展示积分、购买入口或虚构经营数据。

## 18. 2026-09-26 固定一秒剪辑迁移

FR-07 中的卡拍规则只适用于旧项目。新项目保存 `cut_policy: "fixed_1s"`，在 30fps 时间线上每镜恰好取 30 帧；音乐只覆盖全片，不移动切点。旧项目没有该字段时回填为 `beat_aligned`，打开、下载和重新合成仍使用原节拍策略，不自动转换。新项目从脚本阶段把 `Shot.duration_s` 固定为 1，并生成可编辑的 `Shot.caption`；旧镜缺字幕字段时可按空字幕处理。`trim_start_s` 对新项目按 `1/30` 秒吸附，合成用整数帧窗口并在 Worker 验证选段不越过片尾；新项目片头和片尾默认关闭，可在合成设置阶段选择已交付的片头、片尾和音乐。新项目的片段审核完成后进入 `compose_ready`，用户确认设置并按“开始合成”才入队；旧项目保留原直达合成流程。新项目默认开启字幕与转场，可在合成设置页关闭：字幕按每镜的一秒窗口烧录，片头偏移由拼接顺序自然带入；相邻镜头切点前后各 2 帧片内淡出/淡入，总帧数不变。旧逐镜项目在片段审核或成片页可明确选择“使用新版 1 秒剪辑”；转换保留片段，将每镜回到待确认状态，原成片在新成片交付前暂停访问。

## 当前实施补充：2026-09 全站对齐方案

用户确认的《Kelvoy 全站设计对齐与功能补齐计划》是本轮实施基线；与本文早期“比赛期间无积分账户”的表述冲突时，以该计划为准。积分限于团队发放和固定动作计费，不接入购买或支付。

- 新建项目默认 9:16、每镜 3 张候选，允许 1–3 张；仅支持逐镜生成，每镜严格 30 帧、30fps。旧项目保留原候选数和剪辑策略，旧网格项目仅供查看、下载。
- 脚本审核阶段允许重新生成、按指令优化。提交任务时以行版本原子地标记处理中并入队；失败保留原稿并提示重试。镜头字幕可编辑，生成时带出短句。
- 片段审核后进入 `compose_ready` 设置阶段；新项目字幕、2 帧片内淡入淡出默认开启，片头片尾默认关闭。合成需用户明确启动。
- 积分账户初始为 0，由内部 CLI 发放；默认动作价格为脚本 1、图片 1/张、视频 10/段、合成 1。项目创建和阶段提交原子预留，任务成功结算、终态失败释放；同一动作的流水不可变且幂等。任务采用租约，Worker 崩溃后由下一次队列领取恢复。GPU 用量仍作为独立成本估算显示。
- 成片按 `final/<期号>_v<版本>.mp4` 写入，ffprobe 保存时长、宽高、帧率、大小和完成时间。重新编辑或合成时暂停旧交付版本，成功后沿用用户此前的分享开关；分享页支持下载、复制文案和链接、平台入口及二维码。
- 失败重试按数据库里的失败任务入队，使用新动作预留并沿用原生成标识以复用已成功落盘的候选。公开审核接口只允许对应阶段的人为修改字段，不能伪造产物或积分。
- 发布与数据迁移步骤见 `docs/guides/reference-design-migration.md`。DGX 竖横屏真机验收仍需按本文及该指南完成，不能把代码测试视为真机验收。
