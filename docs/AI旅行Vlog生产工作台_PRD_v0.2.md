...
首页保留真实的继续创作、最近作品和本周记录，并提供角色、目的地、模板、审核四个能力入口。目的地卡片的类型筛选取自已入库数据，图片来自相应地标参考图；没有图片时使用缺图状态。官网图片走公开的目的地素材接口，仅提供该目的地目录实际引用的图片，不开放角色参考图。新建一期将原有完整参数分为主要表单、高级设置与角色选择面板，角色按本人和官方分栏；去创建角色再返回时，浏览器会话内保留尚未提交的表单，成功建期后清除。从首页目的地卡片进入时，链接指定的目的地优先于旧草稿里的目的地，其余独立创作字段保留。`/help` 提供与当前功能一致的使用说明。
