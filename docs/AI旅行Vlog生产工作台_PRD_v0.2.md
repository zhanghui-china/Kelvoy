...
一句话定义：一个 AI 旅行 vlog 生产工具——用户定一个虚拟出镜角色，选一个旅游目的地（景区级：一个景区一期），在三个节点做选择（脚本、关键帧、片段），得到一期 24–30 镜、约 30 秒、9:16 的目的地 vlog；同一角色走遍不同目的地，形成一个旅行账号的内容。
...
| 编号 | 阶段 | 需求 | 验收标准 |
| --- | --- | --- | --- |
| FR-01 | Brief | 一期 = 选角色 + 从目的地库选目的地 + 季节 + 语气 + 禁止项 + 关键帧模式（逐镜 / 网格，默认逐镜）；可从模板预填；提交前按 FR-09 粗估成本并展示 | 缺字段给默认值并在项目里标注；估价展示在提交按钮旁 |
...
| FR-11 | 账号 | 用户名密码登录，不开放公开注册——比赛期间共用一个可公网访问的地址，开放注册会让其他参赛队误入项目；账号由团队用 `packages/cli create-user` 预置（见 M2-2、§13），角色列表、期列表、期级成本明细（FR-09）（开放注册、积分账户等设想见附录 A） | 预置账号登录后能看到自己的角色与期 |
