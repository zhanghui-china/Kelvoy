...
>
> 项目目标已调整：**当前阶段是内部比赛（黑客松）展示 demo**，要做的是把核心生产流水线跑通并演示清楚——角色资产、目的地库、六阶段生成、三个人工审核点、成片输出。**商业化逻辑暂时不考虑**：积分计费与支付、ICP / 算法 / 大模型备案、云厂商内容审核接入、冷启动获客、公司主体选择、定价与付费用户指标，都不在当前范围。
>
> 这些内容**没有删除**：它们是团队之前认真推过的，以后若真要产品化还会捡回来。全文凡是标了 **【商业化阶段】** 的条目、行、里程碑，都是面向未来产品化保留的参考，**不是当前要做的事**；读到"上线前""付费用户""备案"这类字眼时按同一口径理解。数据模型（§6）、核心流程（§4）、流水线技术设计（§7）、架构与目录（§9）以及 M0–M2 的技术交付不受本次调整影响。变更明细见文末 §13。

## 1. 产品定义与目标

一句话定义：一个 AI 旅行 vlog 生产工具（"对外销售"是远期产品化定位【商业化阶段】，当前阶段是内部比赛 demo，见文首）——用户定一个虚拟出镜角色，选一个旅游目的地（景区级：一个景区一期），在三个节点做选择（脚本、关键帧、片段），得到一期 24–30 镜、约 30 秒、9:16 的目的地 vlog；同一角色走遍不同目的地，形成一个旅行账号的内容。
...
- 【商业化阶段】上线 8 周内 10 个付费用户（种子来源见第 3 节）——当前比赛 demo 不以付费用户数验收，比赛验收见 §10
- 【当前阶段新增】比赛 demo 验收：同一角色至少 2–3 期不同目的地的成片，能现场走一遍三个审核点并展示成本报告
...
- 团队协作、模板市场（P1）
- 微信 / 手机号登录、系统生成角色参考图、云厂商内容审核 API（P1 / 【商业化阶段】对外上线前）
...
| 形态 | 角色 | 阶段 |
| --- | --- | --- |
| Web 应用 | 产品本体：brief 表单、审片台、进度、成本展示（积分扣费【商业化阶段】）、导出与分享 | P0，M2 |
| CLI | 内部工具：`run <stage> --episode <id>`、`import-destination <json>`、跑模型评测、批量出样片 | M1 先做 |
| Agent / API | 代运营团队批量接入、模板自动化 | P2 |

直说风险：通用"AI 视频生成"是红海。产品成立的前提是垂直到底：只做"虚拟角色 × 旅游目的地"这一种体裁，只对旅游博主账号和文旅机构这一类买家。所以 M0 之前不写 web 代码：先手工出 3 条样片验证方法。【商业化阶段】找 10 个目标用户验证付费意愿（第 10 节）——当前比赛阶段不做付费意愿验证。
...
当前比赛阶段，下表的"用户"与"痛点"仍是 demo 要讲清楚的故事；"付费方式"一列整列属于【商业化阶段】，现在不需要落地。

| 用户 | 痛点 | 用产品做什么 | 付费方式【商业化阶段】 |
| --- | --- | --- | --- |
...
- **角色参考图允许用户上传真人照片**，肖像权与合规责任由用户自行确认并承担；系统不凭空生成与上传图无关的真实人物肖像。当前比赛阶段没有外部用户，团队自己上传的真人照片自己确认授权即可。【商业化阶段】用户协议明确责任划分；备案与合规评估要按这个口径重做，见 §8。（v0.1 的"不生成真人肖像 / 人脸检测拦截真人"改为此条。）
...
- 任一审核点标记"重生成"= 该镜 `status → rejected`，记录 `regen_stage`，worker 从对应阶段重跑；用户报告"坏镜"每镜免费一次、报即生效，之后重生成扣积分【商业化阶段——当前只记录成本，不扣任何东西】。
...
| 编号 | 阶段 | 需求 | 验收标准 |
| --- | --- | --- | --- |
| FR-01 | Brief | 一期 = 选角色 + 从目的地库选目的地 + 季节 + 语气 + 禁止项 + 关键帧模式（逐镜 / 网格，默认逐镜）；可从模板预填；提交前按 FR-09 粗估成本并展示；【商业化阶段】余额不足拦截 | 缺字段给默认值并在项目里标注；估价展示在提交按钮旁 |
...
| FR-09 | 成本 | **当前阶段**：每次调用记录 provider、模型、版本、耗时、费用（见 `shot.model`），期级汇总成"成本报告"（GPU 分钟 + API 费用），提交前粗估 = 镜数 × 候选数 × 单位成本 × 1.5 返工系数、只展示。【商业化阶段】积分按固定汇率换算：1 积分 = X GPU 分钟等价（X 待定），溢出 API 费用按同汇率折算、不加价；超额拦截 | 当前阶段：成本记录与供应商账单 / GPU 时间误差 ≤ 10%；粗估与实际消耗偏差 ≤ 30%（M0 校准常量）。【商业化阶段】积分口径同上 |
| FR-10 | 模板 | 模板对象 = 骨架（六值枚举之一）+ LUT + 片头 + 片尾 + 标题样式，见 §6；官方模板 `owner_id = null`；用户可把当前期的这些存为私有模板；M2 内置三个：大型景区、古镇水乡、名山登顶 | 新的一期从模板起步 ≤ 5 分钟 |
| FR-11 | 账号 | **当前阶段**：用户名密码登录，不开放公开注册——比赛期间共用一个可公网访问的地址，开放注册会让其他参赛队误入项目；账号由团队用 `packages/cli create-user` 预置（见 M2-2、§13），角色列表、期列表、期级成本明细（FR-09）。【商业化阶段】开放注册、微信 / 手机号登录、积分余额与消耗明细、积分包购买 | 当前阶段：预置账号登录后能看到自己的角色与期。【商业化阶段】积分不足在提交前拦截 |
...
账号（User）目前只有登录必需的字段——积分余额与消耗明细（FR-11）属于【商业化阶段】，到时候定完积分汇率再加，现在加只是空占位，不如不加。期上的 `estimated_credits` / `credits_used` 两个字段保留不动：当前阶段填的是成本估算与实际消耗的等价值（口径见 FR-09），不涉及扣费。
...
- 视频环节长期走国内 API 的概率很高；溢出不是兜底，是常态之一，成本报告要如实体现；【商业化阶段】定价要按这个前提算（§8）。
...
- **成本**（当前阶段）：COGS = Spark GPU 时间（折旧 + 电费 + 运维）+ 国内 API 调用费。单期 GPU 时间目标待 M0 定；国内 API 费用上限 ¥20 / 期，口径为整期所有环节都走 API——这条现在是团队自己的预算约束，不是定价依据。
- **毛利 / 积分**【商业化阶段】：积分按固定汇率：1 积分 = X GPU 分钟等价，X 由实测 COGS 按"定价 ≥ 5 × COGS"倒推；溢出 API 费用按同汇率折算计入，不对用户加价。用户超额重生成扣积分，坏镜报告每镜免费重生成一次。
...
- **版本与清理**：角色、目的地带版本号，期记录生成时快照，后续修改不影响老期；用户素材默认 90 天后清理**中间产物**（候选图、网格、未选片段），成片、分镜表、分享链接保留；【商业化阶段】付费用户可延长。
- **合规**（当前阶段保留的部分）：成片带可见 AI 标识 + 文件元数据（模型自带的 SynthID / C2PA 保留不剥离）；AI 标识 MVP 形式：右下角半透明文字"AI 生成 · 虚构角色 · 真实目的地"+ 元数据字段，按《人工智能生成合成内容标识办法》（2025 年 9 月起施行）加显式与隐式标识——成本低、demo 里也该有；不生成他人商标；音乐只用授权曲。
- **备案与合规评估**【商业化阶段】：网站上线前完成 ICP 备案；模型自部署 → 我们自己是模型服务提供者，算法备案 / 大模型备案大概率落在自己头上，对外上线前请专业人士核。**角色参考图允许真人照片**这一决定会加重肖像权与备案侧的审查，合规评估要按 v0.2 口径重做——这是对外上线的前置条件，不是比赛 demo 的前置条件。桌面调研见 docs/M3-0-备案路径调研.md。
- **内容安全**：当前阶段只做 prompt 层关键词拦截（v0.1 的"产物过审、人脸检测"降级）；【商业化阶段】云厂商内容安全 API（文本 + 图片）在对外上线前接入；违规账号封停。
...
团队 2–3 人（产品 / 引擎 / Web，owner 按人定）。**当前比赛范围 = M0 → M1 → M2**，M3 / M4 整体属于【商业化阶段】，原样保留供以后参考。M0 不写一行产品代码——先手工出片，方法不成立后面都不用做（"先找人付钱"这半句归商业化阶段）。M0 行里带【商业化阶段】标记的子项现在跳过，其余技术交付不变。

| 里程碑 | 交付 | 验收 | 工作量 |
| --- | --- | --- | --- |
| M0 需求与方法验证 | 照参考片的方法，用同一个角色手工做无锡 3 期（南长街夜游 / 拈花湾 / 灵山大佛），验证跨期一致性和真实地标还原度；实拍三地参考图建第一份目的地包（景区级三条）；【商业化阶段】访谈 10 个目标用户，记录付费意愿与价格锚点；**在 Spark 上**用大佛、迎客松、清名桥做基准，评测 2 个开源图像 + 2 个开源视频模型 + 1 个国内 API，记录每镜 GPU 分钟数与哪些模型根本跑不动；用 3 期全量做一致性评审集，内部逐镜打分 | 一页选型结论（漂移率、地标还原度、每镜 GPU 分钟、失败率、开源 vs API 并排、Spark 可行性）；估价常量初值（单位成本、返工系数）；【商业化阶段】≥ 3 人愿意预付、积分汇率 X | 1 周 |
| M1 引擎 + CLI | 六阶段流水线、两套状态机、providers（Spark 自部署 + 国内 API 溢出 + 重试链路）、推理服务化、本地 SQLite 存储与任务队列（`packages/store`，ADR-0004）、schema、ffmpeg 合成（卡拍、片头片尾）、费用记录与期级成本报告（积分换算【商业化阶段】）、`import-destination` 录入五个景区，全部命令行 | 第二条片人工操作 ≤ 30 分钟；任一镜可单独重跑；任一期可重新合成 | 1–2 周 |
| M2 Web MVP | 预置账号登录（不开放注册，防其他参赛队误入，见 §13）、成本估价展示（余额拦截【商业化阶段】）、建角色、brief 表单（含模式）、审片台（三个审核点 + 网格查看 + 拖拽选点 + 待审队列）、轮询进度、重新合成、导出、分享链接、3 个内置模板（大型景区 / 古镇水乡 / 名山登顶） | 一个没参与开发的人不经指导完整跑通一条（【商业化阶段】口径为 10 个种子用户） | 2–3 周 |
| 【商业化阶段】M3 付费上线 | 微信 / 支付宝支付、定价页、落地页（同一角色 3 期 + 公开分享链接）、ICP 与模型备案、云厂商内容审核 API 接入、用户协议（含真人参考图责任条款）；冷启动：无锡景区与文旅账号上门 + 小红书 / 抖音旅行创作者社群 | 8 周内 10 个付费用户 | 1–2 周 + 持续运营 |
| 【商业化阶段】M4 模板市场与团队（P1） | 用户模板发布、代运营多客户管理、API、微信登录、系统生成角色参考图、自有店铺核验 | 视 M3 数据决定 | 待定 |

**比赛 Demo 验收**（当前阶段）：同一角色的 3 期 vlog（无锡三地）各带一个分享链接；现场从 brief 到成片走一遍三个审核点，操作 ≤ 30 分钟；成片带 AI 标识和成本报告；任一镜可现场重生成、任一期可现场重新合成。

**上线验收 Demo**【商业化阶段】：落地页上同一角色的 3 期 vlog 各带一个公开分享链接；一个陌生用户从注册到拿到第一条成片 ≤ 30 分钟操作，中途不需要人工指导。

## 11. 风险与开放问题

当前比赛阶段最大的风险是硬件和质量：两台 Spark 的算力决定了视频环节大概率长期走 API，地标失真和角色漂移直接决定 demo 好不好看。【商业化阶段】最大的风险是同质化和获客，不是技术；毛利模型要按"视频全 API"这个前提立。

下表"阶段"列：**当前** = 比赛 demo 就要面对；**商业化** = 对外上线才需要处理，现在只保留。

| 风险 | 影响 | 对策 | 阶段 |
| --- | --- | --- | --- |
| 通用 AI 视频工具红海，大厂原生打包 | 无差异，被免费功能替代 | 只做"虚拟角色 × 真实目的地"这一种体裁；目的地库持续积累 | 商业化（但差异化叙事在 demo 里也要讲清） |
| **Spark 跑不动视频模型** | 视频环节全走 API，成本抬高，¥20 上限吃紧 | M0 第一件事实测；图像 / LLM 留在本地；【商业化阶段】定价按"视频全 API"立 COGS | 当前 |
| 真实地标失真 | 观众和景区一眼看出假，账号信任崩 | 地标镜头必须以实景参考图为条件；质量红线单列；M0 用灵山大佛 / 迎客松 / 清名桥做基准 | 当前 |
| 开源模型质量落后于商用 API | 返工率高 | M0 同一基准并排测；不达标的环节允许长期走 API | 当前 |
| 2 台 Spark 是单点 | 一台宕机容量减半，两台都挂即停服 | 队列超阈值自动溢出国内 API；镜像与权重在云 GPU 上冷备 | 当前（比赛现场至少要有 API 兜底） |
| **允许真人参考图** | 肖像权纠纷、备案审查加重、平台下架 | 当前：只用团队自己确认过授权的照片；【商业化阶段】用户协议明确责任、上线前合规评估按此口径重做；保留随时收紧为"仅生成图"的开关 | 当前（肖像权）/ 商业化（备案） |
| 自部署 = 自己是模型服务提供者 | 备案责任更重 | 与 ICP、支付一起在对外上线前办完 | 商业化 |
| 景区、文旅局对 AI 内容的态度 | 被要求下架，或反过来成为客户 | 先把景区做成客户；片头注明"虚构角色、真实目的地" | 商业化（片头注明这条当前就做） |
| 获客成本高于客单价 | 亏损 | 无锡本地上门 + 样片即广告；文旅机构走年度采购 | 商业化 |
| 内容滥用 | 法律与平台风险 | 当前：关键词拦截 + AI 标识默认开；【商业化阶段】用户协议、对外上线前接云厂商审核 | 当前（最小）/ 商业化 |
| 内容空洞，用户做了没人看 | 留存差 | 模板强制带信息槽位；系列有账号级人设线 | 商业化 |
...
- [ ] 积分汇率 X
- [x] 备案路径：ICP 之外，自部署模型的算法 / 大模型备案怎么办；允许真人参考图对备案的影响——调研见 docs/M3-0-备案路径调研.md（桌面调研初稿，未经专业人士核实）
- [x] 支付与备案主体：用现有个体户还是新设公司——调研见 docs/M3-0-备案路径调研.md，初步倾向新设公司（未经专业人士核实）
- [ ] 定价：按期、积分包还是订阅？文旅机构走年度采购？访谈定锚点
...
| 新建一期页"视频比例 9:16 / 16:9" | NewEpisode.dc.html | §1/§4/FR-04 全文承诺的是 9:16 竖屏 | 当前阶段只做 9:16，brief 表单（#30）不做这个开关 |
