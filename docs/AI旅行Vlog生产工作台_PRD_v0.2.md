...
| FR-11 | 账号 | **当前阶段**：用户名密码注册登录（已实现，见 M2-2）、角色列表、期列表、期级成本明细（FR-09）。【商业化阶段】微信 / 手机号登录、积分余额与消耗明细、积分包购买 | 当前阶段：注册登录后能看到自己的角色与期。【商业化阶段】积分不足在提交前拦截 |
...
| M2 Web MVP | 用户名密码注册登录、成本估价展示（余额拦截【商业化阶段】）、建角色、brief 表单（含模式）、审片台（三个审核点 + 网格查看 + 拖拽选点 + 待审队列）、轮询进度、重新合成、导出、分享链接、3 个内置模板（大型景区 / 古镇水乡 / 名山登顶） | 一个没参与开发的人不经指导完整跑通一条（【商业化阶段】口径为 10 个种子用户） | 2–3 周 |
