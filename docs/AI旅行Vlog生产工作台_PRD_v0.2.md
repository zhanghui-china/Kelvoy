...
本轮产品界面以浅蓝灰背景 `#F3F7FC`、白色主面板、`#F7FAFF` 次面板、深色侧栏 `#1B2434` 和蓝色强调 `#2563FF` 为统一视觉基础。标题与正文使用无衬线字体。桌面侧栏宽 208px；平板以 72px 图标导航展示并保留可访问名称；手机使用 56px 顶栏与菜单抽屉。官网和登录页独立布局，官网首屏使用项目提供的旅行图片。

首页保留真实的继续创作、最近作品和本周记录，并提供角色、目的地、模板、审核四个能力入口。目的地卡片的类型筛选取自已入库数据，图片来自相应地标参考图；没有图片时使用缺图状态。新建一期将原有完整参数分为主要表单、高级设置与角色选择面板，角色按本人和官方分栏；去创建角色再返回时，浏览器会话内保留尚未提交的表单，成功建期后清除。`/help` 提供与当前功能一致的使用说明。
