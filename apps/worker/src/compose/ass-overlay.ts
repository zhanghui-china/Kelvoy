...
export interface TextOverlayEvent { start_s: number; end_s: number; kind: "Title" | "Label" | "Caption"; text: string }

export function textOverlayEvents(plan: ComposePlan, introDurationS: number, outroDurationS: number): TextOverlayEvent[] {
  const total = introDurationS + plan.cuts.reduce((sum, cut) => sum + cut.duration_s, 0) + outroDurationS;
  const events: TextOverlayEvent[] = [];
  const add = (start_s: number, end_s: number, kind: TextOverlayEvent["kind"], text: string) => {
    if (end_s > start_s && text.trim()) events.push({ start_s, end_s, kind, text });
  };
...
export function buildAssOverlay(plan: ComposePlan, introDurationS: number, outroDurationS: number): string {
  ...
  const events = textOverlayEvents(plan, introDurationS, outroDurationS).map((item) =>
    `Dialogue: 0,${assTime(item.start_s)},${assTime(item.end_s)},${item.kind},,0,0,0,,${safeText(item.text)}`);
  return [
